...
export class TranslationsService {
  constructor(
    private addLanguagesCommandHandler: AddTranslationsCommandHandler,
  ...
  async createUpdateTranslationFile(
    ...
    switch (type) {
      case 'json':
        language_list = FileFormationHelper.parseJsonFile(
    ...
    if (language_list[0])
      await this.addLanguagesCommandHandler.execute({
        language_list,
        language,
      });
  }
  ...
  async downloadFile(language, file_name) {
    const type = file_name.split('.').slice(-1)[0];
    const translations = await this.getTranslationsCommandHandler.execute({
      language,
    });
  ...
  async getTranslateById(id: number) {
    ...
    return this.getTranslationsCommandHandler.execute({
